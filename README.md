...
## コラ作成機能
テキストチャンネル上で起動することで、メッセージ内容を画像上に書いたコラ画像を作成できます。\
...
### ミーム画像
テキストチャンネルに送信されたメッセージの文章をミーム画像に張り付けて表示します。

1. スラッシュコマンドによる起動\
    `/collage _meme`

    `/collage _meme id:0123456789 emoji:🤓`\
    *id*および*emoji*オプションは必須になります。
    - id : 画像にするメッセージのID
    - emoji : ミーム画像に対応した絵文字
...
1. スラッシュコマンドによる起動\
    ...
    `/collage _gyotaku_ id:0123456789 emoji:🟥`\
    *id*および*emoji*オプションは必須になります。
    - id : 画像にするメッセージのID
    - emoji : 魚拓画像に対応した絵文字
...
### スパチャ画像
テキストチャンネルに送信されたメッセージの文章に対してスパチャを表示します。

1. スラッシュコマンドによる起動\
    `/collage _superchat`

    `/collage _superchat id:0123456789 emoji:🔴`\
    *id*および*emoji*オプションは必須になります。
    - id : 画像を返信するメッセージのID
    - emoji : スパチャ画像に対応した絵文字

    `/collage _meme id:0123456789 emoji:🔴 amount:hogehoge content:fugafuga`\
    *amount*および*content*オプションを任意で指定できます。
    - amount : スパチャの金額
    - content : スパチャの文章

    指定しない場合、金額はランダム、文章は無言スパチャになります。
...
## その他
「✂️」,「🗑️」,「❌」,「🚮」のリアクションに反応します。\
リアクションを付けたメッセージがすにゃbotのメッセージである場合、そのメッセージの削除を行います。\
リアクションを付けたメッセージがすにゃbotのメッセージでない場合、そのメッセージについているbotのリアクションを削除します。\
メッセージやリアクションを削除する権限を持っていない人が削除できるようにつけてみました。
