...
また以下で示す実装機能に関して個別にhelpコマンドが実装されています。

例1：\
`/cohere_help question`
...
コマンドをチャットに送信することで、非公開で質問ができます。

コマンド：\
`/cohere_question`

例：\
`/cohere_question 好きな人への告白文を考えて`
...
例：\
`/voicevox_setting_server read_sameuser True`

- read_multiline
このオプションを選択すると複数行読み上げの選択肢が出てきます。チャットが二行以上あってもすべて読み上げるようにしたい場合は`True`を選択してください。\
例：\
`/voicevox_setting_server read_multiline True`
...
ただし簡易的に利用できる反面、機能を利用できるまでの手数が増大します。
