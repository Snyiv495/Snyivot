...
function createEmbed(content){
    ...
    switch(content){
        case "start" : {
            ...
            embed.addFields({name: "2️⃣ 複数のチャンネルを読み上げることもできるのだ", value: "読み上げをしてないチャンネルにコマンドを送信すると、読み上げるチャンネルを追加するのだ"});
        ...
        case "setting_user" : {
            ...
            embed.addFields({name: "2️⃣ オプションを追加することで設定の変更ができるのだ", value: "以下のオプションが存在するのだ"});
        ...
        case "setting_server" : {
            ...
            embed.addFields({name: "9️⃣ 他にも読み上げ方のオプションが存在するのだ", value: "詳しくはユーザー設定のヘルプを確認するのだ"});
            embed.setColor(0x00FF00);
            attachment.setName("icon.png");
            attachment.setFile("assets/zundamon/icon/flaunt.png");
            break;
        ...
        case "dictionary_add" : {
            ...
            embed.addFields({name: "2️⃣ オプションを追加することで細かい設定ができるのだ", value: "以下のオプションが存在するのだ"});
            embed.addFields({name: "3️⃣ 読み方を設定する文字を「surface」で指定できるのだ", value: "このオプションは必須なのだ"});
            embed.addFields({name: "4️⃣ 文字の読み方を「pronuncication」で指定できるのだ", value: "このオプションは必須で、カタカナで指定するのだ"});
