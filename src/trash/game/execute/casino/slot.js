...
async function execute(interaction, map){
    ...
    //スロットの送信
    if(!slot_info.bet){
        await interaction.editReply(await createSlot(slot_info, user_info.casino_coin, server_info.casino_slot_jackpot_coin));
        return 0;
    }

    //0レーン停止
    if(!slot_info.left_stop && !slot_info.center_stop && !slot_info.right_stop){
        user_info.casino_coin = user_info.casino_coin - slot_info.bet;
        await db.setUserInfo(interaction.user.id, user_info);
        await turnSlot(interaction, slot_info, map, user_info.casino_coin, server_info.casino_slot_jackpot_coin);
        return 0;
    }
    ...
    if((slot_info.left_stop&&!slot_info.center_stop&&!slot_info.right_stop) || (!slot_info.left_stop&&slot_info.center_stop&&!slot_info.right_stop) || (!slot_info.left_stop&&!slot_info.center_stop&&slot_info.right_stop)){
        await turnSlot(interaction, await checkFirstLane(interaction, slot_info, user_info.casino_coin, server_info.casino_slot_jackpot_coin), map, user_info.casino_coin, server_info.casino_slot_jackpot_coin);
        return 0;
    }

    //2レーン停止
    if((slot_info.left_stop&&slot_info.center_stop&&!slot_info.right_stop) || (slot_info.left_stop&&!slot_info.center_stop&&slot_info.right_stop) || (!slot_info.left_stop&&slot_info.center_stop&&slot_info.right_stop)){
        await turnSlot(interaction, await checkSecondLane(interaction, slot_info, user_info.casino_coin, server_info.casino_slot_jackpot_coin), map, user_info.casino_coin, server_info.casino_slot_jackpot_coin);
        return 0;
    }

    //3レーン停止
    if(slot_info.left_stop && slot_info.center_stop && slot_info.right_stop){
        slot_info = await checkThirdLane(interaction, slot_info, user_info.casino_coin, server_info.casino_slot_jackpot_coin);
        ...
        user_info.casino_coin = user_info.casino_coin + slot_info.payout;
