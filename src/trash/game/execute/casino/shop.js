...
function createModal(user_info){
    ...
    res_buy_coin.setRequired(false);
    ...
    res_buy_ticket.setRequired(false);
...
function createEmbed(user_info){
    ...
    embed.addFields({name: "コイン", value: `${user_info.casino_coin}枚`});
    embed.addFields({name: "チケット", value: `${user_info.casino_ticket}枚`});
...
async function execute(interaction){
    ...
    if(interaction.fields.getTextInputValue("res_buy_coin") && !isNaN(interaction.fields.getTextInputValue("res_buy_coin"))){
        ...
        user_info.casino_coin += Math.max(0, Math.min(num_buy_coin, (user_info.money-(user_info.money%10))/10));
    ...
    if(interaction.fields.getTextInputValue("res_buy_ticket") && !isNaN(interaction.fields.getTextInputValue("res_buy_ticket"))){
        ...
        user_info.casino_ticket += Math.max(0, Math.min(num_buy_ticket, (user_info.casino_coin-(user_info.coin%100))/100));
        user_info.casino_coin -= Math.max(0, Math.min(num_buy_ticket*100, user_info.casino_coin-(user_info.casino_coin%100)));
    }
